---
title: "GSE227136 IPF Lung PreProcessing"
author: "Kendrix"
date: "2025-11-20"
output: 
  html_document:
...
# Load Seurat object.
obj <- readRDS(file = "Z:/Kendrix/IPF_Lung/GSE227136/raw_data/ILD_all_celltypes_Seurat.rds")
...
```{r preliminary QC, out.width="100%"}
...
metadata <- Extract_Sample_Meta(obj,
                                ...
                                variables_include = c("Diagnosis", "Sample_Source", "Sample_Type", "Tobacco", "Status", "Gender", "Age", "Ethnicity"))

# Plot some preliminary sample-level data.
...
# We are only interested in IPF and control, which represent the bulk of the diagnoses in this dataset, so let's subset to include those two only. 
dim(obj) # 28,237; 475,047
subset_obj <- subset(obj, subset = Diagnosis %in% c("IPF", "Control"), invert = FALSE)
dim(subset_obj) # 28,237; 328,429
```

```{r}
# Remove unneeded objects.
rm(col, columns_incl, metadata, obj, p)
```
...
meta %>%
  ggplot(aes(x = manual_annotation_1, fill = Diagnosis)) +
...
Note: Resolution at around 0.6 looks stable based on clustree above. 
...
saveRDS(object = seurat_annotated, 
        file = "Z:/Kendrix/IPF_Lung/GSE227136/processed_data/preprocessed_seurat_obj.rds")
```
