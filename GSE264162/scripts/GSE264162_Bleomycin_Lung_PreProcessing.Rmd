---
title: "GSE264162 Bleomycin Lung PreProcessing"
author: "Kendrix Kek"
date: "2024-06-02"
output:
  html_document:
...
supp_file_path = "/Users/kendrix/Documents/Work/UnityHealth/Data_Projects/scRNAseq_LungFibrosisAnalysis/GSE264162/"
...
```{r run quick Seurat pipeline, message=FALSE, eval=FALSE}
...
```{r calculate nEXP, message=FALSE, eval=FALSE}
...
```{r summarize pN and pK values, out.width="100%", eval=FALSE}
...
```{r run DoubletFinder, eval=FALSE, message=FALSE}
...
```{r PDGFR-a UMAP, fig.height=12, fig.width=15, out.width="100%"}
...
```{r find PDGFRa-cells markers, out.width="100%"}
...
source("/Users/kendrix/Documents/Work/UnityHealth/Data_Projects/technical_scripts/get_conserved_markers.R")
...
```{r PDGFR-b UMAP, fig.height=12, fig.width=15, out.width="100%"}
# Plot the marker expression in the annotated UMAP.
Idents(seurat_cluster) <- "predicted.ann_finest_level"
...
```{r find pericytes markers, out.width="100%"}
# Find the cell counts for pericytes in each treatment.
n_cells <- FetchData(seurat_cluster, 
                     vars = c("predicted.ann_finest_level", "treatment")) %>% 
  dplyr::count(predicted.ann_finest_level, treatment) %>%
...
```{r Krt5 UMAP, fig.height=12, fig.width=15, out.width="100%"}
# Plot the marker expression in the annotated UMAP.
Idents(seurat_cluster) <- "predicted.ann_finest_level"
...
```{r find Krt5+ cells markers, out.width="100%"}
...
```{r final cell type UMAP, fig.height=12, fig.width=15, out.width="100%"}
...
```{r save objects}
save(seurat_cluster, 
     file = "/Users/kendrix/Documents/Work/UnityHealth/Data_Projects/scRNAseq_LungFibrosisAnalysis/GSE264162/data/preprocessed_seurat_obj.RData")
```
